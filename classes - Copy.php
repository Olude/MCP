...
class manageCategories {
	...
	public $id;		// Installation URL Property
	...
	function getCategoriesProfile($type = null) {
	...
	//Functions written by Johnricho
	...
	function getGenres($category) {
		...
		$query = sprintf("SELECT ".$category." FROM users WHERE idu='".$this->id."'");
	...
	function addCategory($value) {
		...
		if($affected) {
			...
			//Alter Users Table and add the new category created as column

			$stmt = $this->db->prepare(sprintf("ALTER TABLE `users` ADD ".strtolower($this->db->real_escape_string($value))." TINYINT(2) NOT NULL DEFAULT '0'"));
...
class feed {
	...
	function getTracks($query, $type, $typeVal) {
		...
		mysql_connect("localhost","jamilbab_sound","ZB30*ugz&=^{");

		mysql_select_db("jamilbab_sound");
		
		$main_author = "<img src='http://unsignedafrika.com/repost_btn_n.png' style='height:17px; vertical-align: middle;'>";



		$topAuthorName = mysql_query("SELECT uid,repost_status,user_id FROM `tracks` WHERE `id` = '".$row['id']."'  ");
		
		 $topAuthorNameData = mysql_fetch_assoc($topAuthorName);
		 
		if( $topAuthorNameData['repost_status']==1)

		{
			$topUserAuthor = mysql_query("SELECT username FROM `users` WHERE `idu` = '".$topAuthorNameData['user_id']."' ");

			$topUserAuthorRow = mysql_fetch_assoc($topUserAuthor);
			
			$nameOfAuthor = $topUserAuthorRow['username'];
			
			$authorLink = $topAuthorNameData['user_id'];
			
			$mainAuthorUsername = mysql_query("SELECT username FROM `users` WHERE `idu` = '".$topAuthorNameData['uid']."' ");
			
			
			$mainAuthorUsernameData = mysql_fetch_assoc($mainAuthorUsername);
			
		
			$main_author .= "<a href='".$this->url."/index.php?a=profile&u=".$mainAuthorUsernameData['username']."'>".$mainAuthorUsernameData['username']." </a>";
...

		{
			...
			$authorLink = $row['idu'];
				...
				$track .= '<div id="track'.$row['id'].'" class="song-container'.(($type == 'trackPage') ? ' song-container-page' : '').'">
						...
						<div class="song-art"><a href="http://unsignedafrika.com/uploads/media/'.$row['art'].'" data-imagelightbox="a"><img src="'.$this->url.'/uploads/media/'.$row['art'].'"style="width:112px;height=112px;" id="song-art'.$row['id'].'"  alt="'.$row['title'].'"></a></div>

						<div class="song-top">
				
							<div class="song-timeago">	'.$main_author.'	</div>
							<br>
							...
							<div class="song-titles"> 
								...
								<div class="song-author"><a onmouseover="profileCard('.$authorLink.', '.$row['id'].', 0, 0);" onmouseout="profileCard(0, 0, 0, 1);" onclick="profileCard(0, 0, 1, 1);" href="'.$this->url.'/index.php?a=profile&u='.$nameOfAuthor.'" rel="loadpage" id="song-author'.$row['id'].'">'.$nameOfAuthor.'</a></div>
							...
							<div class="cover-description">
								...
								<div class="cover-avatar-content">
									...
									<div class="cover-avatar">
										...
										<span id="avatar'.$profile['idu'].$profile['username'].'"><a href="http://unsignedafrika.com/uploads/avatars/'.$coverAvatar.'" data-imagelightbox="a"><img src="'.$this->url.'/uploads/avatars/'.$coverAvatar.'"style="width:112px;height=112px;" /></a></span>
	...
	function updateTrack($values, $type) {
		...
		if($x > 0) {
			...
			while($row = $query->fetch_assoc()) {
				...
				$err .= notificationBox('success', sprintf($LNG['track_uploaded'], $this->url.'/index.php?a=track&id='.$row['id'], $row['title']));
	...
	//Added Function to get Genres and display on the sidebar

	//By Johnricho

	function getUserGenres($type = null) {
		...
		if($type) {
			...
			$query = sprintf("SELECT afrobeatz FROM users U WHERE U.idu='".$this->id."'");
	...
	function getSidebarGenres($category,$id) {

		$category=strtolower($category);		

		$query = sprintf("SELECT ".$category." FROM users WHERE idu='".$id."'");
	...
	//Added Function to get List of Genres by the user and display on the sidebar

	//By Johnricho

	function sidebarGenres($bold,$id) {
		...
		$link = '<div class="sidebar-container widget-categories"><div class="sidebar-content"><div class="sidebar-header">'.$LNG['genres'].'</div>';
		...
		foreach($rows as $category) {
			...
			$query = $this->db->query("SELECT ".$category['name']." FROM users WHERE idu='".$id."'");
	...
	//Added Function to get Gender and display on the sidebar

	//By Johnricho

	function sidebarGender($id) {

		global $LNG;

		

		$link = '<div class="sidebar-container widget-categories"><div class="sidebar-content"><div class="sidebar-header">'.$LNG['gender'].'</div>';
		...
		$query = $this->db->query("SELECT gender FROM users WHERE idu='".$id."'");

		$data = $query->fetch_assoc();			

		$link .= '<div class="sidebar-link"><a href="">'.ucfirst($data['gender']).'</a></div>';
	...
	function sidebarSpecialization($id) {

		global $LNG;

		

		$link = '<div class="sidebar-container widget-categories"><div class="sidebar-content"><div class="sidebar-header">'.$LNG['specialization'].'</div>';
		...
		$query = $this->db->query("SELECT producer,artist,dj FROM users WHERE idu='".$id."'");

		while($data = $query->fetch_assoc()){

			if($data['producer']==1){

				$link .= '<div class="sidebar-link"><a href="">'.ucfirst('Producer').'</a></div>';

			}

			if($data['artist']==1){

				$link .= '<div class="sidebar-link"><a href="">'.ucfirst('Artist').'</a></div>';

			}

			if($data['dj']==1){

				$link .= '<div class="sidebar-link"><a href="">'.ucfirst('DJ').'</a></div>';
			...
			foreach($rows as $row) {
				...
				$output .= '<div class="sidebar-suggestions-inner">
					...
					<div class="sidebar-suggestions">
						...
						<div class="sidebar-suggestions-image"><a href="http://unsignedafrika.com/uploads/avatars/'.$row['art'].'" data-imagelightbox="a"><img src="'.$this->url.'/thumb.php?src='.$row['art'].'&t=m&w=112&h=112" /></a></div>
	...
	function getSidebarMostDownloadSongUser(){

		global $LNG;

		//Get the unsigned artist of the month
...
if($t < "21")
{
		...
		$UnsignedArtist="
					...
					<a   >
						...
						<div class='sidebar-unsigned-artist'>

							<div class='title'>Personality of the Month</div>
<div >This space is for the personality with the most downloads at the end of this month</div>

							<div class='name'></div>

							<div class='picture' style='background:url(question.jpg) no-repeat;background-size:cover;'></div>
		...
		return $UnsignedArtist;

}
else{

		$result = $this->db->query("SELECT *, COUNT(D.track) AS TopDownload, MONTH(D.time) AS DownloadDate FROM downloads D, tracks T, users U WHERE D.track=T.id AND MONTH(D.time)=MONTH(CURDATE()) AND U.idu=T.uid  GROUP BY D.track LIMIT 1");
		...
		$UnsignedArtist="
					...
					<a href=".$this->url."/index.php?a=profile&u=".$artist['username'].">

						<div class='sidebar-unsigned-artist'>

							<div class='title'>Personality of the Month</div>

							<div class='name'>".ucfirst($artist['username'])."</div>

							<div class='picture' style='background:url(".$this->url."/uploads/avatars/".$artist['image'].") no-repeat;background-size:cover;'></div>
	...
	function sidebarButton($id = null, $type = null) {
		...
		if($type == 1) {
			...
			return '<div class="sidebar-button-container"><a href="'.$this->url.'/index.php?a=track&id='.$id.'" rel="loadpage"><div class="edit-button"><span class="success-icon"></span>'.$LNG['view_track'].'</div></a></div>';
				...
				/*added id="uploadBtn" for identification of click by sid*/
			return ($id) ? '<div class="sidebar-button-container"><a href="'.$this->url.'/index.php?a=upload" rel="loadpage" id="uploadBtn"><div class="upload-button"><span class="upload-icon"></span>'.$LNG['upload'].'</div></a></div>' : '<div class="sidebar-container"><div class="sidebar-button-container"><a href="'.$this->url.'/index.php?a=upload" rel="loadpage" id="uploadBtn"><div class="upload-button"><span class="upload-icon"></span>'.$LNG['upload'].'</div></a></div></div>';

		}

	}

	/*Upload album and song popup function by sid*/
	function uploadPopup(){
		$output='<div id="uploadmodal">
					<div class="modal-container">
						<div id="share-title" class="modal-title">
							<div class="modal-menu-container">
								<div id="tab-uploadmodal" class="modal-menu-item modal-menu-item-active">SONG OR ALBUM</div>
								<div class="modal-loading" style="display: none;"></div>
							</div>
						</div>
						<div class="divider"></div>
						<div class="tab-uploadmodal">
							<div class="modal-inner">
								<a href="'.$this->url.'/index.php?a=upload"><div class="upload-button"><span class="upload-icon"></span>Upload Song</div></a>
							</div>
							<div class="modal-inner">';
		$albumBtn = '<a href="'.$this->url.'/index.php?a=upload&type=album"><div class="upload-button"><span class="upload-icon"></span>Upload Album</div></a>';
		
		$gopro = '<a href="'.$this->url.'/index.php?a=pro"><div class="upload-button"><span class="upload-icon"></span>Upload Album</div></a>';
		$output	.= $this->getProStatus($this->id)?$albumBtn:$gopro .'';
		$output	.= '</div><div class="modal-inner">';
	
		$output	.=	'</div></div>	
						<div class="divider"></div>
						<div class="modal-menu">
							<div class="modal-btn"><a onclick="closeUploadmodal()">Close</a></div>
						</div>
					</div>
				</div>';
		return $output;
			...
			foreach($rows as $row) {
				...
				$output .= '<div class="sidebar-suggestions-inner">
					...
					<div class="sidebar-suggestions">
						...
						<div class="sidebar-suggestions-image"><a href="http://unsignedafrika.com/uploads/avatars/'.$row['image'].'" data-imagelightbox="a" title="'.$LNG['profile_view_profile'].'" ><img src="'.$this->url.'/thumb.php?src='.$row['image'].'&t=a&w=112&h=112" /></a></div>
				...
				$output .= '<div class="list-container">
								...
								<div id="subscribe'.$row['idu'].'">'.$this->getSubscribe(0, array('idu' => $row['idu'], 'username' => $row['username'], 'private' => $row['private']), 1).'</div>'.$this->chatButton($row['idu'], $row['username'], 1).'
									...
									<div class="list-avatar" id="avatar'.$row['idu'].'">
										...
										<a href="http://unsignedafrika.com/uploads/avatars/'.$row['image'].'" data-imagelightbox="a">
	...
	function getActions($id, $type = null) {
		...
		$repost = '<div class="repost-button" id="repost_btn'.$id.'" onclick="repostTrack('.$id.')"><span class="action-text">Repost</span></div>';
		...
		$userQuery = mysql_query("SELECT idu FROM `users` WHERE `username` = '".$_SESSION['username']."' ");

		$userRow = mysql_fetch_assoc($userQuery); 

			

		if($userRow['idu'] == $get['uid']) {
		...
		$repostChk = mysql_query("SELECT * FROM `repost` WHERE `repost_by` = '".$userRow['idu']."' and `track` = '".$get['id']."' ");

		if(mysql_num_rows($repostChk) > 0) {
		...
		$sameUserRepost = mysql_query("SELECT * FROM `tracks` WHERE `id` = '".$get['id']."' ");
		
		$sameUserRepostData = mysql_fetch_assoc($sameUserRepost);
		
		if($sameUserRepostData['repost_status']==1 and $sameUserRepostData['user_id']==$userRow['idu'])
		{
			$repost =' ';
		}
		...
		$getUser = mysql_query("SELECT repost_by FROM `repost` WHERE `track` = '".$get['id']."'  ");
...

		{
			...
			while($getUserRow =  mysql_fetch_assoc($getUser))

			{

				$userAuthor = mysql_query("SELECT username FROM `users` WHERE `idu` = '".$getUserRow['repost_by']."' ");

				$userAuthorRow = mysql_fetch_assoc($userAuthor);

				

				$main_author .= "<a href='".$this->url."/index.php?a=profile&u=".$userAuthorRow['username']."'>".$userAuthorRow['username'].','." </a>";

				$num_repost = $num_repost+1;

			$repost_div ="<div class='counter' > <img src='http://unsignedafrika.com/repost_btn_n.png' style='height:17px; vertical-align: middle;'>".$num_repost." </div>";
